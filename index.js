...
app.post("/getdetail", (req, res) => {
  ...
  getData(
    "select DataDateAndTime,Barcode,convert(decimal(10,2),Weight,0)Weight,convert(decimal(10,2),MinWeight,0)MinWeight from DataWeigherID where convert(date,datadateandtime,0)='" +
      filter.Tanggal +
      "' and StatusSorting='" +
      filter.StatusSorting +
      "' order by DataDateAndTime",
    res
  );
});
...
app.post("/getdetail2", (req, res) => {
  ...
  // console.log("select * from DataWeigherID where DataDateAndTime>='"+filter.dari+"' and DataDateAndTime<dateadd(day,1,'"+filter.hingga+"')")
  getData(
    "select DataDateAndTime,Barcode,convert(decimal(10,2),Weight,0)Weight,convert(decimal(10,2),MinWeight,0)MinWeight from DataWeigherID_WithShift where tanggal='" +
      filter.Tanggal +
      "' and StatusSorting='" +
      filter.StatusSorting +
      "' and shift='" +
